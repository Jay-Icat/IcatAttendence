...
const replacement = `function saveDepartmentAttendance(sheet, params) {
  ...
  // Build row index map for students by name and batch FIRST
  var studentRowMap = {};
  ...
  // Determine sessionRowIdx by scanning upwards from first student (minUpdateRow)
  var sessionRowIdx = -1;
  ...
  // Find dateColIdx by scanning ONLY the 6 rows immediately above the sessionRowIdx
  var dateColIdx = -1;
  var scanStartRow = Math.max(0, sessionRowIdx - 6);
  var scanEndRow = sessionRowIdx - 1; 
  ...
  // Determine final target column by scanning horizontally in the sessionRowIdx starting near dateColIdx
  ...
  // Dynamically find Title and Tutor rows by scanning Column B upwards
  var titleRowIdx = -1;
  var tutorRowIdx = -1;
  if (sessionRowIdx > 2) {
